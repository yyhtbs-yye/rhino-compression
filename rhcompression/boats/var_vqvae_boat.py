...
class VARVQVAEBoat(BaseGANBoat):
    """
    Multi-scale VQ-VAE tokenizer training inside BaseGANBoat, with:
    ...
    def _reconstruct(self, x, detach_for_d=False):
        """
        Encode -> quantize (STE) -> decode via model.forward(..., return_dict=True).
        Returns (recon, z_pre, z_quant, vq_loss).
        """
        net = self.models['net']  # expected to be VARVQVAE

        out = net(x, ret_usages=False, clamp=False, return_dict=True)
        recon   = out['recon']
        z_pre   = out['z_pre']
        z_quant = out['z_quant']
        vq_loss = out['vq_loss']
